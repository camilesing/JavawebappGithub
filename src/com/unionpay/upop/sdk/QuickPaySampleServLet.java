...
public class QuickPaySampleServLet extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response , Map<String,Object> Param) {
		//商户需要组装如下对象的数据
		String[] valueVo = new String[]{
	            ...
	            "0",//折扣 单位：分
	            "0",//运费 单位：分
	            Param.get("orderNumber").toString(),//new SimpleDateFormat("yyyyMMddHHmmss").format(new Date())+(++i),//订单号（需要商户自己生成）
	            Param.get("orderAmount").toString(),//"3100",//交易金额 单位：分
	            "156",//交易币种
	            new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()),//交易时间
	            Param.get("customerIp").toString(),//"127.0.0.1",//用户IP
	            Param.get("customerName").toString(),//"张三",//用户真实姓名
	            "",//默认支付方式
	            "",//默认银行编号
	            "300000",//交易超时时间
