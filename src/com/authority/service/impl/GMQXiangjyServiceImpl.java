		...
		try{
			...
			for (Object object : list) {
				...
				if(QTY.toString().equals("0")&&!QTY_QR.equals("0")){ //执行插入操作
					insert = "insert into B_PO_BOXITEM(ID, AD_CLIENT_ID, AD_ORG_ID, B_PO_BOX_ID, B_SO_ID, M_PRODUCT_ID, M_PRODUCTALIAS_ID, M_ATTRIBUTESETINSTANCE_ID, QTY, OWNERID, MODIFIERID, CREATIONDATE, MODIFIEDDATE, ISACTIVE, B_PO_BOXNO_ID,M_SALE_ID) " +
							 "select GET_SEQUENCES('B_PO_BOXITEM') ID,AD_CLIENT_ID, AD_ORG_ID, B_PO_BOX_ID, B_SO_ID, B.M_PRODUCT_ID, B.ID M_PRODUCTALIAS_ID, B.M_ATTRIBUTESETINSTANCE_ID, "+QTY_QR+" QTY, OWNERID, MODIFIERID, CREATIONDATE, MODIFIEDDATE, ISACTIVE, B_PO_BOXNO_ID,M_SALE_ID "+
							 "from B_PO_BOXITEM A  "+
