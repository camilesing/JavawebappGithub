...
import com.authority.web.controller.BOSInterfaceController;



@Service
public class TaskYF_Interface {
	...
	@Autowired
	private BOSInterfaceController bosinterfacecontroller;
	...
	private static final Logger logger = LoggerFactory.getLogger(TaskYF_Interface.class);
	...
	public void datadownload(){
	    ...
	    //    System.out.println("Response ：\n"+strResult);
			...
			if(!ExceptionMsg.trim().equals("")){
				...
				String title = "BOS--EMAX Interface <DATADOWNLOAD> Failed (Release YF)";
	...
	public void dataprocess(){
		...
		try{
			...
			for (Map<String, Object> CusMap : CusList) {
				query = "select count(*) from " +
						"Base_Interface_Task a " +
						"where upper(TaskName)='DATAPROCESS' and Isactive='Y' and Status='0' and upper(CusCode) = :CODE and " +
						"not exists(" +
						"	select 'x' from Base_Interface_Task b " +
						"	where a.TaskOrder > b.TaskOrder and b.Status='1' and b.Isactive='Y' and a.CusCode = b.CusCode " +
						")" ;
				...
				if(count==0)
					...
					//更新任务状态为 1 正在执行中
					update ="update Base_Interface_Task set Status='1' where upper(TaskName)='DATAPROCESS' and Isactive='Y' and upper(CusCode) = :CODE ";
					njdbcTemplate.update(update,CusMap);
				}
				for (File file : arrFiles) {
					...
					query = "select * from BASE_Interface_Condition where isactive='Y' and upper(CusCode) = :CODE";
					...
					WebUtils web = new WebUtils();
					String time = web.readValue("config/others/config.properties","Bos.Stock_Update_Time");
					...
					if(exists&&flag){
						if(TableName.equalsIgnoreCase("M_INVENTORY")){ //库存更新程序 时间控制
					...
					if(exists&&flag&&time_check){
						...
						String result = bosinterfacecontroller.MethodMapping(file, fieldMap, "admin");
						if(!result.equals(""))
							exceptionMsg = result;
					}
			...
			if(!exceptionMsg.equals("")){
				query = "select * from BASE_Interface_Email where isactive='Y' ";
				List<Map<String, Object>> EmailMap = jdbcTemplate.queryForList(query);
				String title = "BOS--EMAX Interface <DATAPROCESS> Failed (Release YF)";
				WebUtils webutils = new WebUtils();
				try {
					for (Map<String, Object> map : EmailMap) {
						String address = map.get("MAIL").toString();
