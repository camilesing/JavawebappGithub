        ...
        for(var ii=0;ii<pdt.length;ii++) {
                    ...
                    for(var ppp=0;ppp<colorArr[p].stores[pp].docnos.length;ppp++){//详细
                        ...
                        for(var con=0;con<4;con++){
                            ...
                            if(con==0){
                                ...
                                //poe 单据号、类型、日期等信息
                                item+="<td rowspan=\"4\" valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-left-txt01\""+(ss=='FWD'?" style='color:blue;word-break:break-all;'":"")+">"+pdt[ii].xmlns+"</td>"+
                                      "<td rowspan=\"4\" valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-left-txt01\""+(ss=='FWD'?" style='color:blue;word-break:break-all;'":"")+">"+colorArr[p].stores[pp].docnos[ppp].date+"</td>"+
                                ...
                                for(var w=0;w<colorArr[p].stores[pp].docnos[ppp].tag.size.length;w++){
                                    ...
                                    if(w<colorArr[p].stores[pp].docnos[ppp].tag.size.length-1){
                                    	...
                                    	//alert(qtyAl);
                                    	item+="<td valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-bg\""+(itemMetrixTr=='non'?" style=\"background-color:#eeeeee\"":"")+">"+(itemMetrixTr!='non'?"<input id='"+pdt[ii].xmlns+"-"+colorArr[p].name+"-"+colorArr[p].stores[pp].name+"-boxcnt'  qtyId='"+pdt[ii].xmlns+"' x-index='"+this.get_cell_x_index(w)+"' y-index='"+(style_y_length-1)+"' title='"+barCode+"' color=\""+colorArr[p].name+"\"  size=\""+colorArr[p].stores[pp].docnos[ppp].tag.size[w]+"\" barCode='"+barCode+"' docNo=\""+docno+"\" store=\""+colorArr[p].stores[pp].name+"\" sty=\""+pdt[ii].xmlns+"\" name=\""+docno+"\" type=\"text\" class=\"td-txt-input\" value=\""+(qtyAl==0?1:qtyAl)+"\"/>":"")+"</td>";
                                    }
                                    //edit by Robin 2010.5.7 在配货的input中增加属性：barCode,sty,docNo,store。。。
                                }
                                //poe 小计
                                item+="<td id='"+(pdt[ii].xmlns+colorArr[p].name+docno+colorArr[p].stores[pp].name)+"' boxQty='"+docno+"' valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-right-txtW\">"+v0+"</td>";
	                            ...
	                            if(boxCnt>0){
                               	...
                               	//总计
                                item+="<td id='"+(pdt[ii].xmlns+colorArr[p].name+docno+colorArr[p].stores[pp].name)+"cnt' valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-right-txtW\">"+v0+"</td>";
                            ...
                            if(con==1){
                                ...
                                for(var w=0;w<colorArr[p].stores[pp].docnos[ppp].tag.size.length;w++){
                                    ...
                                    if(w<colorArr[p].stores[pp].docnos[ppp].tag.size.length-1){
                                    	...
                                    	item+="<td id='"+pdt[ii].xmlns+"-"+this.get_cell_x_index(w)+"-"+(style_y_length-1)+"已配' valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-right-txtD\""+(itemMetrixTr=='non'?" style=\"background-color:#eeeeee\"":" docType='"+ss+"'")+">"+(itemMetrixTr!='non'?itemMetrixTr:"")+"</td>";
                                    }else {
                                    	item+="<td valign=\"top\" rowspan=\"3\" bgcolor=\"#8db6d9\" class=\"td-right-txtW\"></td>";
                                    }
                                }
                            ...
                            if(con==2){
                                ...
                                for(var w=0;w<colorArr[p].stores[pp].docnos[ppp].tag.size.length;w++){
                                    ...
                                    if(w<colorArr[p].stores[pp].docnos[ppp].tag.size.length-1){
                                    	...
                                    	item+="<td id='"+pdt[ii].xmlns+"-"+this.get_cell_x_index(w)+"-"+(style_y_length-1)+"余量' valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-right-txtW\""+(itemMetrixTr=='non'?" style=\"background-color:#eeeeee\"":" id='"+(docno+barCode)+"-rem' docType='"+ss+"'")+">"+(itemMetrixTr!='non'?itemMetrixTr:"")+"</td>";
                                    }
                                }
                            ...
                            if(con==3){
                                ...
                                for(var w=0;w<colorArr[p].stores[pp].docnos[ppp].tag.size.length;w++){
                                    ...
                                    if(w<colorArr[p].stores[pp].docnos[ppp].tag.size.length-1){
                                    	...
                                    	item+="<td id='"+pdt[ii].xmlns+"-"+this.get_cell_x_index(w)+"-"+(style_y_length-1)+"库存' valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-right-txtK\""+(itemMetrixTr=='non'?" style=\"background-color:#eeeeee\"":" name='"+barCode+"' title='"+itemMetrixTr+"' docType='"+ss+"'")+">"+(itemMetrixTr!='non'?itemMetrixTr:"")+"</td>";
                                    }
                                }
            ...
            */
            var tot_qty_al_for_col="<tr>"+
                    "<td bgcolor=\"#FFFFFA\" class=\"td-left-title\" colspan=\"5\">款已配量合计</td>";
            var tot_for_style=0;
    		...
    		*/
    		...
    		while(i>=0){
    			if(dataForTemp[i].barCode==barcode&&docType==dataForTemp[i].docType){
    				...
    				var boxCnt = parseInt(jQuery("#"+sty+"-"+color+"-"+store+"-boxcnt").val(),10);
    				totAlForBarCode = totAlForBarCode*boxCnt;
...
//      			if(eCnt+realQtyAlChange>boxQty){
//      				alert("单箱数量"+boxQty+"不符，请重新输入！");
//      				var oldQtyAl = cellData.oldQtyAl;
//      				var x_index = cellData.x_index;
//      				var y_index = cellData.y_index;
//      				jQuery("#"+cellData.sty+"-"+x_index+"-"+y_index).val(oldQtyAl);
//      				e.html(eCnt+"");
//      				return;
//      			}
      			//原合计
      			var total =jQuery("#"+cellData.qtyId+cellData.color+cellData.docNo+cellData.store+"cnt");
      			//获取箱数
    ...
    update_row_box_count:function(cellData,realQtyAlChange){
    		if(cellData.docNo&&cellData.store&&cellData.color){
    	//	alert(cellData.docNo+cellData.store+cellData.color);
    		//	alert(realQtyAlChange);
      			var e =jQuery("#"+cellData.qtyId+cellData.color+cellData.docNo+cellData.store);
...
//                	}
		           ...
		           // var nowQty=parseInt(jQuery(this).val(),10);//响应事件后的数量
                	...
                	//获取款号
                   	...
                   	if(jQuery(this).attr("size")!='箱数'){
                   		...
                   		var boxCount = parseInt(jQuery("#"+boxSty+"-"+boxColor+"-"+boxStore+"-boxcnt").val(),10);
               		 	var qtyAlChange=(nowQty-oldQtyAl)*boxCount;//改变的数量
               		 	var barCodeCanDist=dist.get_barcode_can_dist(jQuery(this).attr("barCode"),jQuery(this).attr("docType"));//可分配量
	                    //nowQty*boxCount>qtyRem||
	                    ...
	                    var e =jQuery("#"+cellData.qtyId+cellData.color+cellData.docNo+cellData.store);
		      			...
		      			if(eCnt+nowQty-oldQtyAl>boxQty){
                   		...
                   		//判断箱数是否为零
               			var boxCount = parseInt(jQuery(this).val(),10);
               			var newTotalBoxCnt = 0;
               			if(isNaN(boxCount)?0:boxCount>0){
               				var boxStyle = jQuery(this).attr("sty");
               				var boxHtml = jQuery("#"+boxStyle+"箱数");
               				var oldTotalBoxCnt = 0;
               				//获取原箱数款合计
               				for(var i=0;i<dist.dataforbox.length;i++){
					            if(boxStyle?dist.dataforbox[i].sty==boxStyle:true){
	               		...
	               		for(var index=0;index<parseInt(x_index);index++){
	               			...
	               			var doType = jQuery("#"+boxSty+"-"+index+"-"+y_index).attr("docType");
	               			var barCodeChangeDist=dist.get_barcode_can_dist(baCode,doType);
	               			var sizeValueChange = sizeValue*(nowQty-oldQtyAl);//改变的量
	               			//sizeValue*nowQty>qtyRem||
	               			if(sizeValueChange>barCodeChangeDist) {
	               				dist.tonext=1;
		                    	jQuery(this).focus();
		                    	dist.i=this;
		                        alert("当前可配量已小于0！请重新分配！");
		                    ...
		                    cellData.sty=boxSty;
		                    cellData.docType=doType;
		                    cellData.box = '1';
		                    cellData.size=jQuery("#"+boxSty+"-"+index+"-"+y_index).attr("size");
		                    
		                    //var realQtyAlChange=newQtyAl-oldQtyAl;//实际单元配货量改变数
		                   	dist.update_when_cell_real_change(cellData,sizeValueChange);
	               			//获取当前尺寸库存量
	               			//var wareSizeCnt = parseInt(jQuery("#"+boxSty+"-"+index+"-"+y_index+"库存").html(),10);
	               		}
	               		//更新箱数
	               		var cellDataBox={};
				        ...
				        cellDataBox.qtyAl=jQuery(this).val(); //现在的配货量
    ...
    update_when_cell_real_change:function(cellData,realQtyAlChange){
        ...
        //cellData2.oldQtyAl=cellData.oldQtyAl;
        //cellData2.x_index=cellData.x_index;
        //cellData2.y_index=cellData.y_index;
      	//  alert(realQtyAlChange); 
      	...
      	if(sizeStr!='箱数'){
      		if(cellData.box=='0'){
				...
				var count = parseInt(jQuery("#"+cellData.sty+"-"+cellData.color+"-"+cellData.store+"-boxcnt").val(),10);
				var boxCnt = isNaN(count)?0:count;
				realQtyAlChange = parseInt(realQtyAlChange/boxCnt,10);
      			// 可编辑单元数量改变更新小计 		
				...
				this.update_style_tot_al_for_col(cellData.size,cellData.sty,realQtyAlChange);
      			...
      			this.update_row_box_count(cellData2,realQtyAlChange);
      		}
      	}else{
      		this.update_row_box_count(cellData2,realQtyAlChange);
      	}
