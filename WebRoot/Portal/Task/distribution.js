...
DIST.prototype={
    initialize: function() {
        ...
        this.dataForQtyCan={};//订单余量可用量JSON对象
        this.dataForQtyRem={};//订单库存可用量JSON对象
        this.dataForQtyDest={};//已配量JSON对象
        this.dataForStyleQtyAl={};//所有款已配量的json对象
        this.dataForStyBar={};//barcode对应的款号json
        ...
        this.tonext=0;
        this.totalAllCnt=new Array();//总已配
        this.dataforbox=new Array();//总箱数
...
//        var m_allot_id=$("fund_balance").value||-1;
        ...
        for(var i=0;i<this.data.length;i++){
        	if(parseInt(this.data[i].qtyAl,10)>=0){
        		if(this.data[i].size=='箱数'){
        			var ii={};
	        		ii.qty_ady=this.data[i].qtyAl;
	        		ii.sty=this.data[i].sty;
	        		ii.color=this.data[i].color
	        		ii.m_product_alias_id=this.data[i].bardcode_id;
	        		ii.store=this.data[i].store;
	        		ii.docNo=this.data[i].docNo;
	        		ii.b_so_matchsize_id=this.data[i].b_so_matchsize_id;
	        		box_item.push(ii);
        		}else{
        			var ii={};
	        		ii.qty_ady=this.data[i].qtyAl;
...
//        param.notes=$("orderNotes").value.strip()||$("notes").value.strip()||"";
        ...
        param_box.m_item = (box_item.length==0?"null":box_item);
        
        var evt = {};
        evt.param = Object.toJSON(param);
        evt.param_box = Object.toJSON(param_box);
...
//            var message = "111,121915,125521";
//            var array = message.split(",");
//            alert(array[0]);
//            if(array.length>1){
//            	for(var i=0;i<array.length-1;i++){
//            		jQuery("left_"+array[i]).attr("style","");
//            		jQuery("left_"+array[i]).attr("style","background-color:red");
//            	}
//            }
            $("isChanged").value="false";
            window.location=window.location.href;
            //window.location.href = "/penta/index.jsp";
        });
    },
    reShow:function(){
        var evt={};
...
//        var data=e.getUserData();
      ...
      //  alert(Object.toJSON(ret.data));
        ...
        for(var ii=0;ii<pdt.length;ii++) {
            ...
            this.manuStr+="\n<li><div id=\"left_"+pdt[ii].xmlns+"\" class=\"txt-on\"  onclick='javascript:$(\"pdt-img\").src = \"/pdt/"+pdt[ii].M_PRODUCT_LIST+"_1_2.jpg\";" ;
             if(pdt[ii].value=="\"\""){
            ...
            for(var p=0;p<colorArr.length;p++){//色号
                ...
                for(var pp=0;pp<colorArr[p].stores.length;pp++){//店仓
                	var store_length = colorArr[p].stores.length;//当前色号下店仓length
                    for(var ppp=0;ppp<colorArr[p].stores[pp].docnos.length;ppp++){//详细
                        ...
                        for(var w=0;w<colorArr[p].stores[pp].docnos[ppp].tag.size.length;w++){//尺寸
                             ...
                             */                      
                             ...
                             var qtyDest= colorArr[p].stores[pp].docnos[ppp].tag.dest[w]!='non'?parseInt(colorArr[p].stores[pp].docnos[ppp].tag.dest[w],10):0;//订单量
                          	 var bardcode_id= colorArr[p].stores[pp].docnos[ppp].tag.bardcode_id[w];//单元格ID 唯一
                          var cellData={};//每个单元的数据，即配货编辑的单元                      
                          if(colorArr[p].stores[pp].docnos[ppp].tag.barCode[w]!='non'){
                            this.dataForStyBar[colorArr[p].stores[pp].docnos[ppp].tag.barCode[w]]=pdt[ii].xmlns;//款号
							...
							cellData.b_so_matchsize_id = colorArr[p].stores[pp].docnos[ppp].b_so_matchsize_id; //配码
                            ...
                            cellData.bardcode_id= bardcode_id;
                        ...
                        for(var con=0;con<4;con++){
                            ...
                            if(con==0){
                                ...
                                //poe 单据号、类型、日期等信息
                                item+="<td rowspan=\"4\" valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-left-txt01\""+(ss=='FWD'?" style='color:blue;word-break:break-all;'":"")+">"+colorArr[p].stores[pp].docnos[ppp].date+"</td>"+
                                      "<td rowspan=\"4\" valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-left-txt01\""+(ss=='FWD'?" style='color:blue;word-break:break-all;'":"")+">"+colorArr[p].stores[pp].docnos[ppp].no+"</td>"+
                                      "<td rowspan=\"4\" valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-left-txt01\""+(ss=='FWD'?" style='color:blue;word-break:break-all;'":"")+">"+pdt[ii].xmlns+"</td>"+
                                      "<td rowspan=\"4\" valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-left-txt01\""+(ss=='FWD'?" style='color:blue;word-break:break-all;'":"")+">"+colorArr[p].stores[pp].docnos[ppp].product_name+"</td>"+
                                ...
                                for(var w=0;w<colorArr[p].stores[pp].docnos[ppp].tag.size.length;w++){
                                    ...
                                    var qtyCan = itemMetrixTr!='non'?parseInt(itemMetrixTr,10):0;
                                    var docno=colorArr[p].stores[pp].docnos[ppp].no;
                                    var boxqty=colorArr[p].stores[pp].docnos[ppp].boxqty;
                                    var b_so_matchsize_id=colorArr[p].stores[pp].docnos[ppp].b_so_matchsize_id;
                                    qtyAl=isNaN(qtyAl)?0:qtyAl;
                                    qtyRem=isNaN(qtyRem)?0:qtyRem;
                                    if(w<colorArr[p].stores[pp].docnos[ppp].tag.size.length-1){
                                    	v0+=qtyAl;
                                    	//modify penta input添加store_length属性
                                    	item+="<td valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-bg\""+(itemMetrixTr=='non'?" style=\"background-color:#eeeeee\"":"")+">"+(itemMetrixTr!='non'?"<input id='"+pdt[ii].xmlns+"-"+this.get_cell_x_index(w)+"-"+(style_y_length-1)+"' x-index='"+this.get_cell_x_index(w)+"' y-index='"+(style_y_length-1)+"' qtyCan='"+qtyCan+"' b_so_matchsize_id='"+b_so_matchsize_id+"' store_length='"+store_length+"' qtyId='"+pdt[ii].xmlns+"' qtyRem='"+qtyRem+"' qtyDest='"+qtyDest+"' title='"+barCode+"' barCode='"+barCode+"' color=\""+colorArr[p].name+"\"  size=\""+colorArr[p].stores[pp].docnos[ppp].tag.size[w]+"\" docNo=\""+docno+"\" store=\""+colorArr[p].stores[pp].name+"\" sty=\""+pdt[ii].xmlns+"\" name=\""+docno+"\" type=\"text\" docType='"+ss+"' class=\"td-txt-input\" value=\""+(qtyAl==0?'':qtyAl)+"\"/>":"")+"</td>";
                                    }else{
                                    	//alert(qtyAl);
                                    	item+="<td valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-bg\""+(itemMetrixTr=='non'?" style=\"background-color:#eeeeee\"":"")+">"+(itemMetrixTr!='non'?"<input id='"+pdt[ii].xmlns+"-"+colorArr[p].name+"-"+colorArr[p].stores[pp].name+"-"+docno+"-"+b_so_matchsize_id+"-boxcnt'  qtyId='"+pdt[ii].xmlns+"' qtyCan='"+qtyCan+"' x-index='"+this.get_cell_x_index(w)+"' b_so_matchsize_id='"+b_so_matchsize_id+"' y-index='"+(style_y_length-1)+"' title='"+barCode+"' color=\""+colorArr[p].name+"\"  size=\""+colorArr[p].stores[pp].docnos[ppp].tag.size[w]+"\" barCode='"+barCode+"' docNo=\""+docno+"\" store=\""+colorArr[p].stores[pp].name+"\" sty=\""+pdt[ii].xmlns+"\" name=\""+docno+"\" type=\"text\" class=\"td-txt-input\" value=\""+(qtyAl==0?0:qtyAl)+"\"/>":"")+"</td>";
                                    }
                                    //edit by Robin 2010.5.7 在配货的input中增加属性：barCode,sty,docNo,store。。。
                                }
                                var b_so_matchsize_id=colorArr[p].stores[pp].docnos[ppp].b_so_matchsize_id;
                                var boxqty_total = colorArr[p].stores[pp].docnos[ppp].boxqty_total; //配码*箱数 总数量
                                item+="<td id='"+(pdt[ii].xmlns+colorArr[p].name+docno+colorArr[p].stores[pp].name+b_so_matchsize_id)+"' boxQty='"+boxqty+"' boxqty_total='"+boxqty_total+"' valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-right-txtW\">"+v0+"</td>";
                               	var boxIndex = colorArr[p].stores[pp].docnos[ppp].tag.size.length-1;
	                            var boxIndexCnt=parseInt(colorArr[p].stores[pp].docnos[ppp].tag.qtyAl[boxIndex],10);
	                            var boxCnt = isNaN(boxIndexCnt)?0:boxIndexCnt;
                            	v0 = v0*boxCnt;
                               	//总计
                                item+="<td id='"+(pdt[ii].xmlns+colorArr[p].name+docno+colorArr[p].stores[pp].name+b_so_matchsize_id)+"cnt' valign=\"top\" style=\"color:#FF0033\" class=\"td-right-txtW\">"+v0+"</td>";
                                
                                tot_all_cnt += v0;
                            }
                            if(con==1){
                                 var v3=0;
                                item+="<td valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-right-txt\""+(ss=='FWD'?" style='color:blue;'":"")+">已配量</td>";
                                for(var w=0;w<colorArr[p].stores[pp].docnos[ppp].tag.size.length;w++){
                                    var itemMetrixTr=colorArr[p].stores[pp].docnos[ppp].tag.dest[w];
                                    var barCode=colorArr[p].stores[pp].docnos[ppp].tag.barCode[w];
                                    if(w<colorArr[p].stores[pp].docnos[ppp].tag.size.length-1){
                                    	v3+=isNaN(parseInt(itemMetrixTr,10))?0:parseInt(itemMetrixTr,10);
                                    	item+="<td id='"+pdt[ii].xmlns+"-"+this.get_cell_x_index(w)+"-"+(style_y_length-1)+"已配' qtyDest=\""+barCode+"已配\" valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-right-txtD\""+(itemMetrixTr=='non'?" style=\"background-color:#eeeeee\"":" docType='"+ss+"'")+">"+(itemMetrixTr!='non'?itemMetrixTr:"")+"</td>";
                                    }else {
                                    	item+="<td valign=\"top\" rowspan=\"2\" bgcolor=\"#8db6d9\" class=\"td-right-txtW\"></td>";
                                    }
                                }
                                item+="<td valign=\"top\" rowspan=\"2\" bgcolor=\"#8db6d9\" class=\"td-right-txtW\"></td>";
                                item+="<td valign=\"top\" rowspan=\"2\" bgcolor=\"#8db6d9\" class=\"td-right-txtW\"></td>";
                            ...
                            if(con==2){
                                var v2=0;
                                var boxCnt=0;
                                item+="<td valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-right-txt\""+(ss=='FWD'?" style='color:blue;'":"")+">参考库存</td>";
                                for(var w=0;w<colorArr[p].stores[pp].docnos[ppp].tag.size.length;w++){
                                    ...
                                    if(w<colorArr[p].stores[pp].docnos[ppp].tag.size.length-1){
                                    	...
                                    	item+="<td id='"+pdt[ii].xmlns+"-"+this.get_cell_x_index(w)+"-"+(style_y_length-1)+"参考库存' valign=\"top\" barcode='"+barCode+"库存' bgcolor=\"#8db6d9\" class=\"td-right-txtW\""+(itemMetrixTr=='non'?" style=\"background-color:#eeeeee\"":" id='"+(docno+barCode)+"-rem' docType='"+ss+"'")+">"+(itemMetrixTr!='non'?itemMetrixTr:"")+"</td>";
                                    }
                                }
                            ...
                            if(con==3){
                            	var v1=0;
                                var boxCnt=0;
                                item+="<td valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-right-txt\""+(ss=='FWD'?" style='color:blue;'":"")+">订单配码</td>";
                                for(var w=0;w<colorArr[p].stores[pp].docnos[ppp].tag.size.length;w++){
                                    var itemMetrixTr=colorArr[p].stores[pp].docnos[ppp].tag.can[w];
                                    var barCode=colorArr[p].stores[pp].docnos[ppp].tag.barCode[w];
                                    if(w<colorArr[p].stores[pp].docnos[ppp].tag.size.length-1){
                                ...
                                item+="<td valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-right-txtW\">"+v1*boxCnt+"</td>";
                            }
                            item+="</tr>";
                        }
                    }
            ...
            var tempdivsize="<table id='divtable"+pdt[ii].xmlns+ "'  border=\"0\" cellpadding=\"0\" cellspacing=\"1\"  bgcolor=\"#8db6d9\""+(ii!=0?" style='display:none;table-layout:fixed;position:relative;z-index:3;top:22px;top: expression(this.offsetParent.scrollTop);'":" style='table-layout:fixed;position:relative;z-index:3;top:22px;top: expression(this.offsetParent.scrollTop);'")+">";
            ...
            var tempcol= this.forTableShowStyle(tagLen)+"<tr'>"+
                          ...
                          "<td bgcolor=\"#FFFFFF\" class=\"td-left-title\">经销商</td>" +
                          "<td bgcolor=\"#FFFFFF\" class=\"td-left-title\">日期</td>" +
                          "<td bgcolor=\"#FFFFFF\" class=\"td-left-title\">订单号</td>" +
                          "<td bgcolor=\"#FFFFFF\" class=\"td-left-title\">款号</td>" +
                          "<td bgcolor=\"#FFFFFF\" class=\"td-left-title\">品名</td>"+
            ...
            */
            var tot_qty_al_for_col="<tr>"+
                    "<td bgcolor=\"#FFFFFA\" class=\"td-left-title\" colspan=\"7\">款已配量合计</td>";
            var tot_for_style=0;
            for(var e=0;e<sizeArr.length;e++){
            	//alert(sizeArr.length); //poe  取出尺寸台头
                ...
                if(sizeArr[e]!='箱数'){
                	...
                	var dataforbox_child = {} // sty ,qty 
					dataforbox_child.sty=pdt[ii].xmlns;
					dataforbox_child.qty=col_tot;
					dist.dataforbox.push(dataforbox_child);
                }
            }
//            //统计全部总计
//	        var totalAllCnt_child = {} // sty ,qty 
//			totalAllCnt_child.sty=pdt[ii].xmlns;
//			totalAllCnt_child.qty=v0;
//			dist.totalAllCnt.push(totalAllCnt_child);
//            var sizeName = sizeArr[sizeArr.length - 1];
//            var count = jQuery("#"+pdt[ii].xmlns+sizeName).val();
//            var boxCnt =isNaN(count)?0:count;
            tot_qty_al_for_col+="<td valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-right-txtW\" ID='style-"+pdt[ii].xmlns+"'>"+tot_for_style+"</td>"+
            					"<td valign=\"top\" bgcolor=\"#8db6d9\" class=\"td-right-txtW\" ID='style-"+pdt[ii].xmlns+"-total'>"+tot_all_cnt+"</td>"+
                                "</tr>";  // poe  款已配量合计该栏最后的总合计信息
            this.itemStr+="<td bgcolor=\"#B6D0E7\" class=\"td-right-title\">小计</td>";
            tempdivsize+="<td bgcolor=\"#B6D0E7\" class=\"td-right-title\">小计</td>";
            this.itemStr+="<td bgcolor=\"#B6D0E7\" class=\"td-right-title\">总计</td>";
            tempdivsize+="<td bgcolor=\"#B6D0E7\" class=\"td-right-title\">总计</td>";
            this.itemStr+="</tr>";
            tempdivsize+="</tr></table>"
            this.itemStr+=item;  //poe 加入具体的内容信息  款号、色号、尺寸库存等信息
         ...
         for(var m=0;m<tempdivsizeArray.length;m++){
    	   ...
    	   window.onscroll=function(){
			    ...
			    if( jQuery("#currSty").val()!=""){		
			     var tablehight = jQuery("#currSty").height();
			    ...
			    if(scrollTop>=78){
			      divsizelayer.style.top = scrollTop-69  + 'px'; 
			     }else{
    ...
    */    
    ...
    init_data_for_bankcolor:function(){
	     ...
	     if(array.length>1){
        	for(var i=0;i<array.length-1;i++){
        		jQuery("#left_"+array[i]).attr("style","");
        		jQuery("#left_"+array[i]).addClass("red");
        	}
        }
        /*
    	for(var key in this.dataForQtyRem){
    		if(this.dataForQtyRem[key]<0){
    			jQuery("td[barCode='"+key.replace("FWD","")+"库存']").addClass("red");
    		}
  			jQuery("td[barCode='"+key.replace("FWD","")+"库存']").html(this.dataForQtyRem[key]+"");
    	}
    	for(var key in this.dataForQtyDest){
  			jQuery("td[qtyDest='"+key.replace("FWD","")+"已配']").html(this.dataForQtyDest[key]+"");
    	}
    	* */
    	for(var key in this.dataForQtyRem){
    		var QtyRem = this.dataForQtyRem[key];
    		var barCode_KC = key.replace("FWD","")+"库存";
    		var TableID = this.dataForStyBar[key.replace("FWD","")];
    		if(QtyRem<0){
    			jQuery("#"+TableID + " td[barCode='"+barCode_KC+"']").addClass("red");
    		}
  			jQuery("#"+TableID + " td[barCode='"+barCode_KC+"']").html(QtyRem+"");
    	}
    	for(var key in this.dataForQtyDest){
    		var QtyDest = this.dataForQtyDest[key]
    		var barCode_YP = key.replace("FWD","")+"已配"; 
    		var TableID = this.dataForStyBar[key.replace("FWD","")];
  			jQuery("#"+ TableID + " td[qtyDest='"+barCode_YP+"']").html(QtyDest+"");
    	}
    },
    /**
    ...
    init_data_for_qtycan:function(){
    	...
    	while(dataForTemp.length>0){
    		...
    		var qtyRem=parseInt(dataForTemp[0].qtyRem,10);
    		var qtyDest=parseInt(dataForTemp[0].qtyDest,10);
    		/*
    		数组逐渐减少，循环遍历，index递减
    		*/
    		/*
    		...
    		while(i>=0){
    			if(dataForTemp[i].barCode==barcode&&docType==dataForTemp[i].docType){
    				var sty=dataForTemp[i].sty;
		    		var color=dataForTemp[i].color;
		    		var store=dataForTemp[i].store;
		    		var docNo=dataForTemp[i].docNo;
		    		var boxHtml = jQuery("#"+sty+"-"+color+"-"+store+"-"+docNo+"-boxcnt").val();
		    		var boxCnt = parseInt(boxHtml,10);
		    		if(boxCnt>0){
	    				//对已配量进行合计
						totAlForBarCode+=parseInt(dataForTemp[i].qtyDest,10);
		    		}
    		...
    		/*end*/
    		/*
    		this.dataForQtyRem[barcode+docType]=qtyRem-totAlForBarCode;
    		this.dataForQtyCan[barcode+docType]=qtyCan-totAlForBarCode;
    		this.dataForQtyDest[barcode+docType]=totAlForBarCode;
    	}*/
    	//采用数据库取值方式
     ...
     */    
        ...
        for(var i=0;i<data.length;i++){
            if((cellData.b_so_matchsize_id?data[i].b_so_matchsize_id==cellData.b_so_matchsize_id:true)
            &&(cellData.sty?data[i].sty==cellData.sty:true)
            &&(cellData.color?data[i].color==cellData.color:true)
            &&(cellData.size?data[i].size==cellData.size:true)
            &&(cellData.docNo?cellData.docNo==data[i].docNo:true)
            &&(cellData.store?cellData.store==data[i].store:true)
            &&(cellData.barCode?cellData.barCode==data[i].barCode:true)){
                result.push(data[i]);
            }
        }
        return result;			
    ...
    update_v2mdata_celldata:function(cellData){
        if(cellData.sty&&cellData.color&&cellData.b_so_matchsize_id&&cellData.docNo&&cellData.store&&cellData.barCode){
    		for(var i=0;i<this.data.length;i++){
    			if(cellData.b_so_matchsize_id==this.data[i].b_so_matchsize_id
    			&&cellData.docNo==this.data[i].docNo
    			&&cellData.store==this.data[i].store
    			&&cellData.barCode==this.data[i].barCode
    			&&cellData.sty==this.data[i].sty&&cellData.color==this.data[i].color){
     ...
     * @param docType 订单类型
     */
    get_barcode_rem_dist:function(barCode,docType){
      if(barCode){
//      	if(!this.priorityFutures){
//      		if(this.dataForQtyRem[barCode+'INS']<this.dataForQtyRem[barCode+'FWD'])
//      			this.dataForQtyRem[barCode+'FWD']=this.dataForQtyRem[barCode+'INS'];
//      		else if(this.dataForQtyRem[barCode+'INS']>this.dataForQtyRem[barCode+'FWD'])
//      			this.dataForQtyRem[barCode+'INS']=this.dataForQtyRem[barCode+'FWD'];
//      	}
      	//alert(this.dataForQtyRem[barCode+docType]);
      	return parseInt(this.dataForQtyRem[barCode+docType],10);
      }else{
      	return 0;
      }
    },
    ...
    update_barcode_can_dist:function(barCode,docType,realQtyAlChange){
    	...
    	var b=parseInt(this.dataForQtyRem[barCode+docType],10)-realQtyAlChange;
    	//this.dataForQtyCan[barCode+docType]=a;
		this.dataForQtyRem[barCode+docType]=b;
    },
    /**
     * edit by Robin 2010.5.7
     * 计算总单已配量
     ...
     */
    update_row_count:function(cellData,realQtyAlChange,boxCnt){
    		if(cellData.docNo&&cellData.store&&cellData.color){
    		//	alert(realQtyAlChange);
    			//更新小计
      			var e =jQuery("#"+cellData.qtyId+cellData.color+cellData.docNo+cellData.store+cellData.b_so_matchsize_id);
      			var eCnt = parseInt(e.html(),10);
      			e.html(eCnt+realQtyAlChange+"");
      			//原合计
      			var total =jQuery("#"+cellData.qtyId+cellData.color+cellData.docNo+cellData.store+cellData.b_so_matchsize_id+"cnt");
      			//获取箱数
    			//var count = parseInt(jQuery("#"+cellData.sty+"-"+cellData.color+"-"+cellData.store+"-boxcnt").val(),10);
      			//var boxCnt = isNaN(count)?0:count;
      			//更新合计
      			total.html((parseInt(e.html(),10)*boxCnt)+"");
      			//原总合计
      			var totalAllCntHtml = jQuery("#style-"+cellData.sty+"-total");
      			var total_All_Cnt = parseInt(totalAllCntHtml.html(),10);
      			total_All_Cnt+=realQtyAlChange*boxCnt;//总已配
      			//更新全部总合计
      			jQuery("#style-"+cellData.sty+"-total").html(total_All_Cnt+"");
        }
    },
    update_row_box_count:function(cellData,realQtyAlChange,boxCnt){
    		if(cellData.docNo&&cellData.store&&cellData.color){
    	//	alert(cellData.docNo+cellData.store+cellData.color);
    		//	alert(realQtyAlChange);
      			...
      			//获取原先总计值
      			var total =jQuery("#"+cellData.qtyId+cellData.color+cellData.docNo+cellData.store+cellData.b_so_matchsize_id+"cnt");
      			var oldTotal = parseInt(total.html(),10);
      			//获取箱数
      			//var count = parseInt(jQuery("#"+cellData.sty+"-"+cellData.color+"-"+cellData.store+"-boxcnt").val(),10);
      			//var boxCnt = isNaN(count)?0:count;
      			//更新总计值
      			total.html((parseInt(e.html(),10)*boxCnt)+"");
      			//获取更新后总计值
      			var tot = parseInt(total.html(),10);
      			//原总合计
      			var totalAllCntHtml = jQuery("#style-"+cellData.sty+"-total");
      			var total_All_Cnt = parseInt(totalAllCntHtml.html(),10);
      			total_All_Cnt +=tot - oldTotal;
      			//更新全部总合计
      			jQuery("#style-"+cellData.sty+"-total").html(total_All_Cnt+"");
        }
    },
    ...
    forItemDocnoChangeToArr:function(itemDocno){
    	...
    	if(this.checkIsArray(itemDocno)){
          for(var jj=0;jj<itemDocno.length;jj++){
              ...
              docnoArr[jj].b_so_matchsize_id = itemDocno[jj].b_so_matchsize_id;
              docnoArr[jj].type = itemDocno[jj].sotype;
              docnoArr[jj].product_name = itemDocno[jj].product_name;
              docnoArr[jj].boxqty = itemDocno[jj].boxqty;
              docnoArr[jj].boxqty_total = itemDocno[jj].boxqty_total;
              ...
              docnoArr[0].b_so_matchsize_id = itemDocno.b_so_matchsize_id;
              docnoArr[0].type = itemDocno.sotype;
              docnoArr[0].product_name = itemDocno.product_name;
              docnoArr[0].boxqty = itemDocno.boxqty;
              docnoArr[0].boxqty_total = itemDocno.boxqty_total;
    ...
    forItemTagChangeToArr:function(itemTag){
      ...
      if(this.checkIsArray(itemTag)){
          for(var s=0;s<itemTag.length;s++) {
              ...
              bardcode_id[s]=itemTag[s].content?itemTag[s].BARCODE_ID:'non';
          }
      }else{
          ...
          bardcode_id[0]=itemTag[s].content?itemTag[s].BARCODE_ID:'non';
      }
      tag.size=size;
      tag.can=can;
      tag.rem=rem;
...
//                	}
		            ...
		            if(event.which==8||event.which==46||event.which==32){
                	 ...
                	 var cellData1={};
				            ...
				            cellData1.b_so_matchsize_id=jQuery(this).attr("b_so_matchsize_id");
		            var dataCell1=dist.v2m_get_ret(cellData1)[0];
		            var oldQtyAl=parseInt(dataCell1.qtyAl,10);//未编辑前的单元的已配量
		           // var nowQty=parseInt(jQuery(this).val(),10);//响应事件后的数量
                	...
                	//获取款号
                   	var boxSty =  jQuery(this).attr("sty");
                   	
                   	//var qtyRem=parseInt(dataCell1.qtyRem,10);
                   
                   	if(jQuery(this).attr("size")!='箱数'){
                   		var boxColor = jQuery(this).attr("color");
	               		var boxStore = jQuery(this).attr("store");
	               		var docno = jQuery(this).attr("docno");
	                    ...
	                    cellData.qtyAl=nowQty;
	                    cellData.oldQtyAl=oldQtyAl;
	                    cellData.sty=jQuery(this).attr("sty");
	                    cellData.docType="FWD";
	                    cellData.size=jQuery(this).attr("size");
	                    cellData.x_index=jQuery(this).attr("x-index");
	                    cellData.y_index=jQuery(this).attr("y-index");
	                    cellData.box = '0';
	                    
	                    var e =jQuery("#"+cellData.qtyId+cellData.color+cellData.docNo+cellData.store+cellData.b_so_matchsize_id);
		      			var eCnt = parseInt(e.html(),10);
		      			//判断单箱数量是否符合
		      			var boxQty = parseInt(e.attr("boxQty"));
		      			if(eCnt+nowQty-oldQtyAl>boxQty){
		      				dist.i=this;
		      				dist.tonext=1;
		      				alert("单箱数量_"+boxQty+"_不符，请重新输入！");
		      				jQuery("#"+cellData.sty+"-"+cellData.x_index+"-"+cellData.y_index).val(oldQtyAl);
		      				e.html(eCnt+"");
		      				return;
		      			}
		      			...
		      			if((eCnt+nowQty-oldQtyAl)*boxCount>boxqty_total){
		      				//dist.i=this;
		      				//dist.tonext=1;
		      				alert("配码总数量超标("+(eCnt+nowQty-oldQtyAl)*boxCount+"/"+boxqty_total+",请重新输入！(配码)");
		      				jQuery("#"+cellData.sty+"-"+cellData.x_index+"-"+cellData.y_index).val(oldQtyAl);
		      				e.html(eCnt+"");
		      				return;
		      			}
		      			...
		      			var nowQtyCan = oldQtyCan - qtyAlChange*boxCount; 
		      			...
		      			var nowQtyDest = oldQtyDest + qtyAlChange*boxCount; 
		      			jQuery("#"+boxSty+" td[qtyDest='"+cellData.barCode+"已配']").html(nowQtyDest+"");
	                   	dist.update_when_cell_real_change(cellData,qtyAlChange);
                   	}else{
                   		//判断箱数是否为零
                   		var newTotalBoxCnt = 0;
               			if(nowQty>=0){
               				var boxHtml = jQuery("#"+boxSty+"箱数");
               				var oldTotalBoxCnt = 0;
               				//获取原箱数款合计
               				for(var i=0;i<dist.dataforbox.length;i++){
					            if(boxSty?dist.dataforbox[i].sty==boxSty:true){
					               oldTotalBoxCnt =dist.dataforbox[i].qty;
					            }
					        }
           				newTotalBoxCnt = oldTotalBoxCnt+nowQty-oldQtyAl;
	               		}
	               		//判断配码箱数是否超标
	                    ...
	                    cellData.store=jQuery(this).attr("store");
               			cellData.b_so_matchsize_id=jQuery(this).attr("b_so_matchsize_id");
	                    var e =jQuery("#"+cellData.sty+cellData.color+cellData.docNo+cellData.store+cellData.b_so_matchsize_id);
		      			var eCnt = parseInt(e.html(),10);
		      			var boxqty_total = parseInt(e.attr("boxqty_total"));
		      			if(eCnt*nowQty>boxqty_total){
                   		...
                   		//获取索引
	               		var x_index = jQuery(this).attr("x-index");
	               		var y_index = jQuery(this).attr("y-index");
                   		//当输入箱数时，判断是否超量
						...
						cellDataBox.barCode=jQuery(this).attr("barCode");
				        cellDataBox.docNo = jQuery(this).attr("docNo");
				        cellDataBox.store=jQuery(this).attr("store");
				        cellDataBox.b_so_matchsize_id=jQuery(this).attr("b_so_matchsize_id");
				        cellDataBox.qtyAl=parseInt(jQuery(this).val(),10); //现在的配货量
		    			dist.update_v2mdata_celldata(cellDataBox);//更新单元
		    			
		    			//设置新的箱数款合计
	       				for(var i=0;i<dist.dataforbox.length;i++){
				            if(boxSty?dist.dataforbox[i].sty==boxSty:true){
				               dist.dataforbox[i].qty = newTotalBoxCnt;
				            }
				        }
				        boxHtml.html(newTotalBoxCnt+"");
                   	}
    ...
    update_when_cell_real_change:function(cellData,realQtyAlChange){
        ...
        cellData2.sty=cellData.sty;
        cellData2.b_so_matchsize_id=cellData.b_so_matchsize_id;
      	//  alert(realQtyAlChange); 
      	var sizeStr = cellData.size;
      	//获取箱数
		var count = parseInt(jQuery("#"+cellData.sty+"-"+cellData.color+"-"+cellData.store+"-"+cellData.docNo+"-"+cellData.b_so_matchsize_id+"-boxcnt").val(),10);
		var boxCnt = isNaN(count)?0:count;
      	if(sizeStr!='箱数'){
      		if(cellData.box=='0'){
      			// 可编辑单元数量改变更新小计 		
				this.update_row_count(cellData2,parseInt(realQtyAlChange,10),boxCnt);
				//更新列合计
				this.update_style_tot_al_for_col(cellData.size,cellData.sty,parseInt(realQtyAlChange,10));
				if(boxCnt>0){
					//更新款总已配量
