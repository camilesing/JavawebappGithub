...
%>
<%
	String b_plan_id = request.getParameter("b_plan_id");
	//查询款号、商品ID、商品名
	...
	for(int i=0;i<styleList.size();i++){
		...
		for(int g=0;g<colorList.size();g++){
			...
			//取经销商List
			List storeList = QueryEngine.getInstance().doQueryList("select d.id,d.name from M_ISSUE_APPLYITEM a,m_attributesetinstance c,m_product b,C_CUSTOMER d where a.M_ISSUE_APPLY_ID="+b_plan_id+
	         " and a.m_attributesetinstance_id=c.id and a.m_product_id=b.id and b.name='"+styList.get(0)+"' and c.value1_code ="+color.get(0)+
	         " and a.c_customer_id = d.id  group by d.name,d.id ");
			for(int store=0;store<storeList.size();store++){
				...
				w_docno.put("billdate", styList.get(2));
				...
				List sizeList = QueryEngine.getInstance().doQueryList("select a.* from (select c.value2_code as content,a.id,"+
						" sum(a.QTY) AS QTY_SO,sum(a.QTY*decode(nvl(a.BOXQTY,1),0,1,nvl(a.BOXQTY,1))) AS DESTQTY,sum(a.SOQTY) AS QTYCAN,sum(a.STOCKQTY) as QTYREM,sum(a.QTY) AS QTY_ALLOT"+
						" from M_ISSUE_APPLYITEM a,m_product b,m_attributesetinstance c,m_size d"+
						" where a.M_ISSUE_APPLY_ID='"+b_plan_id+"'"+
						" and a.m_product_id=b.id and b.name = '"+styList.get(0)+"' and c.value1_code = '"+color.get(0)+"' and a.c_customer_id="+stoList.get(0)+
						" and a.m_attributesetinstance_id=c.id"+
						" and c.value2_id=d.ID"+
						" group by  c.value2_code,a.id order by c.value2_code ASC) a "+
						" union all select distinct '箱数' ,to_number(b.ID||c.ID),null,null,null,null,a.BOXQTY from"+
...
jQuery(document).ready(function(){dist._onLoadMetrix();});
</script>
